...
for specimen in os.listdir(ROOT_DIR):
  ...
  for block in os.listdir(specimen_dir):
    ...
    if not os.path.exists(reference_thumbnail) or not os.path.exists(moving_thumbnail):
    ...
    radius = [5, 5, 5]
    closed_image = sitk.BinaryMorphologicalClosing(binary_image, radius, structuring_element)
    ...
    if not os.path.exists(reference_multi_chunk):
      subprocess.run('image_graph_cut -u 1.2 -n 100 -c 4 0.1 {}/reference_binary_mask.nii.gz {}/reference_multi_chunk.nii.gz {}'.format(registration_dir,
                                                                                                                                        registration_dir,
                                                                                                                                        N_CHUNKS),
    ...
    if not os.path.exists(output_global_rigid):
      subprocess.run('greedy -d 2 -a -dof 6 -i {} {} -gm {} -ia-image-centers -m WNCC 4x4 -wncc-mask-dilate -n 200x200x40x0x0 -search 20000 any 10 -o {}'.format(reference_scalar,
                                                                                                                                                                moving_scalar,
    ...
    if not os.path.exists(output_piecewise_rigid):
      subprocess.run('multi_chunk_greedy -d 2 -a -dof 6 -i {} {} -cm {} -ia {} -m WNCC 4x4 -wncc-mask-dilate -n 600x600x200x0 -search 10000 10 5 -wreg 0.05 -o {}'.format(reference_scalar,
                                                                                                                                                                          moving_scalar,
    ...
    output_piecewise_deformable = f"{registration_dir}/output_piecewise_deformable_%02d.nii.gz"
    if not os.path.exists(output_piecewise_deformable):
      subprocess.run('multi_chunk_greedy -d 2 -i {} {} -cm {} -it {} -m WNCC 4x4 -wncc-mask-dilate -n 400x200x100x20 -sv -s 0.6mm 0.1mm -e 0.25 -o {}'.format(reference_scalar,
                                                                                                                                                              moving_scalar,
                                                                                                                                                              reference_multi_chunk,
                                                                                                                                                              output_piecewise_rigid,
    ...
    if not os.path.exists(registration_output):
      subprocess.run('multi_chunk_greedy -d 2 -rf {} -cm {} -r {} {} -rb 255 -rm {} {}'.format(reference_thumbnail,
                                                                                              reference_multi_chunk,
                                                                                              output_piecewise_deformable,
                                                                                              output_piecewise_rigid,
                                                                                              moving_thumbnail,
    ...
    if not os.path.exists(result_workspace):
      subprocess.run('itksnap-wt -layers-add-anat {} -psn "Registered Tau" -layers-add-anat {} -psn "Nissl" -o {}'.format(registration_output,
                                                                                                                          reference_thumbnail,
                                                                                                                          result_workspace),
                                                                                                                          shell=True, stdout=subprocess.DEVNULL)
